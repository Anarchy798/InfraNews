...
RSS_FEEDS = [
    ...
    # General business (filtered for infra/Europe)
...
EUROPE_KEYWORDS = {
    ...
    "luxembourg", "slovenia", "croatia", "serbia", "ukraine", "türkiye", "turkey",
...
def fetch_all() -> list[dict]:
    ...
    for feed_cfg in RSS_FEEDS:
        ...
        try:
            feed = feedparser.parse(url, agent="InfraNews/1.0 (+https://github.com/anarchy798/infranews)")
...
def analyse_article(art: dict, client: anthropic.Anthropic) -> dict:
    prompt = PROMPT_TEMPLATE.format(
        title=art["title"],
        source=art["source"],
        published=art["published"],
        snippet=art["raw_summary"] or "(no snippet)",
    )
    try:
...
def main() -> None:
    ...
    articles = fetch_all()
    log.info("Found %d relevant articles after filtering", len(articles))

    if not articles:
        log.warning("No articles found — exiting without writing news.json")
        return

    log.info("=== Analysing with Claude (%s) ===", CLAUDE_MODEL)
    results: list[dict] = []
    for i, art in enumerate(articles):
        log.info("[%d/%d] %s", i + 1, len(articles), art["title"][:70])
        results.append(analyse_article(art, client))

    # Sort: PE score descending, then newest first
    results.sort(key=lambda a: (-a["pe_score"], a["published"]), reverse=False)
